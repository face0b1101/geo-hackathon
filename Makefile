...
.PHONY: setup setup-no-service-user deploy-ilm deploy-indices deploy-enrich deploy-pipelines deploy-kibana \
        deploy-cases deploy-workflows deploy-agents deploy-demouser deploy-cockpit deploy-es deploy-ai redeploy \
...
deploy-cockpit:     ## Deploy Cockpit dashboard (Vega PFD)
	./setup.sh --only cockpit $(FORCE_FLAG)
